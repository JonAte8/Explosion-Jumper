...
public class PlayerSecondJumpState : PlayerBaseState
{
    float jumpForce = 1000;
    float runSpeed = 500;
    float accelerationSpeed = 30;

    public PlayerSecondJumpState(PlayerStateMachine currentContext, PlayerStateFactory playerStateFactory, string currentStateName)
   : base(currentContext, playerStateFactory, currentStateName) { }

    public override void EnterState()
    {
        ctx.Jump_Velocity = jumpForce * ctx.groundNormal;
        ctx.anim.SetBool("Jump2", true);
        ctx.jumpCounter++;
