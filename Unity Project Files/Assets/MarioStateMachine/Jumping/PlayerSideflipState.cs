...
public class PlayerSideflipState : PlayerBaseState
{
    ...
    public PlayerSideflipState(PlayerStateMachine currentContext, PlayerStateFactory playerStateFactory, string currentStateName)
   : base(currentContext, playerStateFactory, currentStateName) { }

    public override void EnterState()
    {
        ctx.Jump_Velocity = jumpForce * ctx.groundNormal;
        ctx.anim.Play("Sideflip", 0);
        ctx.anim.SetBool("Sideflip", true);
        ctx.jumpCounter = 1;
    ...
    {
        ctx.anim.SetBool("Sideflip", false);
    }
    ...
    public override void GravityFunction()
    {
        ...
        ctx.Ground_Velocity = Vector3.MoveTowards(ctx.Ground_Velocity, ctx.transform.forward * forwardSpeed, forwardSpeed);
