...
public class PlayerFirstJumpState : PlayerBaseState
{
    float jumpForce = 800;
    float runSpeed = 500;
    float accelerationSpeed = 30;

    public PlayerFirstJumpState(PlayerStateMachine currentContext, PlayerStateFactory playerStateFactory, string currentStateName)
   : base(currentContext, playerStateFactory, currentStateName) { }

    public override void EnterState() {
        ctx.Jump_Velocity = jumpForce * ctx.groundNormal;
        ctx.anim.SetBool("Jump1", true);
        ctx.jumpCounter++;
